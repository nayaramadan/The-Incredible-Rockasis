...
TELEM 19 bytes type=0x02 | roll f32 | pitch f32 | yaw f32 | flags u8 |
               sonar u16 | pan u8×100 | CRC16 LE
...
class DriveState:
    ...
    THROTTLE_STEP = 0.1
    STEERING_STEP = 0.1
    DECAY         = 0.85   # per-tick decay when no key held
...
def draw(stdscr, rover: RoverLink, drive: DriveState,
    ...
    put(22, 4, f"CMD rate {cmd_hz:.1f} Hz", curses.color_pair(5))

    # ── Controls legend ───────────────────────────────────────────────────────
    put(24, 2, "─── Keys ────────────────────────────", curses.color_pair(5))
    put(25, 4, "W/S  Throttle    A/D  Steering", curses.color_pair(5))
    put(26, 4, "Space  Stop      C    Toggle claw", curses.color_pair(5))
    put(27, 4, "Q / Esc   Quit", curses.color_pair(5))
